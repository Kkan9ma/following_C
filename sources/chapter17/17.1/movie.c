...
void read_file(struct movie movie_list[], int* ptr_n_items) // 파일을 읽어들이면서 배열에 유효하게 저장되어 있는 아이템의 개수
{
	...
	int num;
	if (fscanf(file, "%d%*c", &num) != 1) { // 읽어보고 오류 있으면 탐지.
...
{
	for (int n = 0; n < n_items; ++n)
		printf("%d : \"%s\", %.1f\n", n + 1, movie_list[n].title, movie_list[n].rating);
}
...
int main(void)
{
	...
	size_t n_items = 0; // 실제로 추가된 아이템의 개수
